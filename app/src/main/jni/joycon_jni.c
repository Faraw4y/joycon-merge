...
/* Config — updated from Java side via setConfig() */
typedef struct {
    int stick_fuzz;
    int stick_flat;
    ...
    /* Right Joy-Con button codes → output */
    int code_a, map_a;
    int code_b, map_b;
    int code_x, map_x;
    int code_y, map_y;
    int code_r,  map_r;
    int code_zr, map_zr;
    int code_plus, map_plus;
    int code_r3,  map_r3;
    /* Left Joy-Con button codes → output */
    int code_l,   map_l;
    int code_zl,  map_zl;
    int code_minus, map_minus;
    int code_l3,  map_l3;
    /* dpad codes */
...
static Config cfg = {
    .stick_fuzz = 256,
    .stick_flat = 4096,
    .inv_lx=0,.inv_ly=0,.inv_rx=0,.inv_ry=0,
    .left_axis_x=0,.left_axis_y=1,
    .right_axis_x=3,.right_axis_y=4,
    .code_a=304,.map_a=0x130,  /* BTN_SOUTH */
    .code_b=305,.map_b=0x131,  /* BTN_EAST  */
    .code_x=307,.map_x=0x133,  /* BTN_NORTH */
    .code_y=308,.map_y=0x134,  /* BTN_WEST  */
    .code_r=0x136,.map_r=0x136,   /* BTN_TR   */
    .code_zr=0x137,.map_zr=0x137, /* BTN_TR2  */
    .code_plus=0x13b,.map_plus=0x13b, /* BTN_START */
    .code_r3=0x13d,.map_r3=0x13d,    /* BTN_THUMBR */
    .code_l=0x135,.map_l=0x135,   /* BTN_TL   */
    .code_zl=0x139,.map_zl=0x139, /* BTN_TL2  */
    .code_minus=0x13a,.map_minus=0x13a, /* BTN_SELECT */
    .code_l3=0x13c,.map_l3=0x13c,      /* BTN_THUMBL */
...
static int find_joycons(void) {
    ...
    if (!dir) { LOGE("opendir failed"); return -1; }
    struct dirent *entry;
    ...
    closedir(dir);
    if (!left_path[0])  { notify_status("ERROR: Left Joy-Con not found");  return -1; }
    if (!right_path[0]) { notify_status("ERROR: Right Joy-Con not found"); return -1; }
    char msg[128];
...
static void handle_right(struct input_event *ev) {
    if (ev->type == EV_KEY) {
        ...
        if (ev->code == cfg.right_axis_x) {
            if (cfg.inv_rx) v = -v;
            v = clamp(v, -32768, 32767);
            emit(EV_ABS, ABS_RX, v);
        } else if (ev->code == cfg.right_axis_y) {
            if (cfg.inv_ry) v = -v;
            v = clamp(v, -32768, 32767);
            emit(EV_ABS, ABS_RY, v);
        }
    } else if (ev->type == EV_SYN) {
        emit(EV_SYN, SYN_REPORT, 0);
    }
...
static int setup_uinput(void) {
    ...
    if (uinput_fd < 0) { LOGE("open uinput failed"); return -1; }
    ...
    for (int i=0;i<4;i++) {
        ...
        uidev.absfuzz[axes[i]] = cfg.stick_fuzz;
        uidev.absflat[axes[i]] = cfg.stick_flat;
    }
    uidev.absmin[ABS_HAT0X]=-1; uidev.absmax[ABS_HAT0X]=1;
    uidev.absmin[ABS_HAT0Y]=-1; uidev.absmax[ABS_HAT0Y]=1;

    if (write(uinput_fd, &uidev, sizeof(uidev)) < 0) return -1;
...
Java_com_joyconmerge_MergeService_setConfig(JNIEnv *env, jobject thiz,
    jint fuzz, jint flat,
    jint invLX, jint invLY, jint invRX, jint invRY,
    ...
    jint dpUp, jint dpDown, jint dpLeft, jint dpRight)
{
    ...
    cfg.inv_lx=invLX; cfg.inv_ly=invLY;
    cfg.inv_rx=invRX; cfg.inv_ry=invRY;
    cfg.left_axis_x=laxX; cfg.left_axis_y=laxY;
    cfg.right_axis_x=raxX; cfg.right_axis_y=raxY;
    cfg.code_a=cA; cfg.map_a=mA;
    cfg.code_b=cB; cfg.map_b=mB;
    cfg.code_x=cX; cfg.map_x=mX;
    cfg.code_y=cY; cfg.map_y=mY;
    cfg.code_r=cR; cfg.map_r=mR;
    cfg.code_zr=cZR; cfg.map_zr=mZR;
    cfg.code_plus=cPlus; cfg.map_plus=mPlus;
    cfg.code_r3=cR3; cfg.map_r3=mR3;
    cfg.code_l=cL; cfg.map_l=mL;
    cfg.code_zl=cZL; cfg.map_zl=mZL;
    cfg.code_minus=cMinus; cfg.map_minus=mMinus;
    cfg.code_l3=cL3; cfg.map_l3=mL3;
...
Java_com_joyconmerge_MergeService_startMerge(JNIEnv *env, jobject thiz) {
    ...
    if (left_fd < 0) { notify_status("ERROR: Cannot open Left Joy-Con"); return -1; }
    ioctl(left_fd, EVIOCGRAB, 1);

    right_fd = open(right_path, O_RDONLY);
    if (right_fd < 0) { notify_status("ERROR: Cannot open Right Joy-Con"); return -1; }
    ioctl(right_fd, EVIOCGRAB, 1);

    if (setup_uinput() < 0) { notify_status("ERROR: Cannot create virtual device"); return -1; }
